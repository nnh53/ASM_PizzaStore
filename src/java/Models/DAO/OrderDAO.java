...
public class OrderDAO {
    ...
    public String generateID() throws Exception {
        ...
        id = DaoUltil.parseNumberToID(count + 1, "O");
        cn.close();
        return id;
    }
    ...
    public Order addOrder(Order orderToAdd) throws Exception {
        //fix cứng ID tự tăng
        orderToAdd.setOrderID(this.generateID());
        //2.add vô db
        Connection cn = DBConnection.getConnection();
        int af = 0;
    ...
    public ArrayList<Order> getAll() throws Exception {
        ...
        Connection cn = DBConnection.getConnection();
        ResultSet rs = null;
        String sql = "SELECT OrderID,CustomerID,ShipAdress,OrderDate,Status FROM dbo.[Order] WHERE [Status] != 'disabled'";
        rs = DBConnection.getResultSetFromQuery(cn, sql); //truyền đúng tham số theo sql ko là đi
        //2.parse/map result
        Order orderRS = null;
